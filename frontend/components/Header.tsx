...
import { Menu, X, User, Droplet, Users, Search as SearchIcon, LogOut } from 'lucide-react';
...
function HeaderAccountButton({ account }: { account: { address: string; displayBalance?: string } }) {
  ...
  const hasNickname = nicknames[account.address.toLowerCase()];
...
function MobileConnectButton() {
  ...
  return (
    <button
      onClick={() => open({ view: 'Account' })}
      className="w-full py-3 rounded-xl bg-teal-500 text-white font-bold shadow-lg shadow-teal-500/20 active:scale-95 transition-all focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900"
      aria-label={`Manage wallet: ${displayName}`}
    >
      {displayName}
    </button>
  );
