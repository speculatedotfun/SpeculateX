  ...
  return (
    <div className={`relative flex items-center justify-center ${className}`}>
      <motion.span
        ...
        className={`font-black tracking-tight transition-colors duration-500 ${priceClassName} ${direction === 'up'
            ? 'text-emerald-500'
            : direction === 'down'
              ? 'text-rose-500'
              : ''
      ...
      <AnimatePresence>
        {direction && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className={`absolute inset-0 rounded-full blur-xl pointer-events-none ${direction === 'up' ? 'bg-emerald-400' : 'bg-rose-400'
              }`}
          />
        )}
      </AnimatePresence>
